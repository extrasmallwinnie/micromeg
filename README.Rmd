...
I've made up an example study where nasal swabs were taken from people who were either "healthy" or "sick" at the time of sampling. We've collected their health status, their age at collection, sex, and simple location (rural or urban). There is also one lab negative control and one lab positive control. Let's load it into our session as an object called "metadata":
...
2. Location is missing for the participant that sample "HC1" was taken from. Let's say that the participant declined to share their location, so that's why it's missing from our data.    
