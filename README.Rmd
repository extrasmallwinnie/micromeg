...
Next, let's looks at the example ASV count table. (N.B.: The data doesn't necessarily strictly have to be an ASV table. Any sort of data in tabular format, e.g., OTU table, similar to the example **should** work.)
...
Finally, let's look at the example taxonomy file. This was also generated during the dada2 workflow, and must match the ASV table.
...
At this point in the tutorial, the existence of all the example in a single list isn't that helpful yet, but will become more so later. Later on, I'll demonstrate a function that you can use to list-ify your own data in a similar manner. This will allow you to have to type less, which is always nice.
