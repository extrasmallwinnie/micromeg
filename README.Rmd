...
This is the least "bad" warning. We got a warning that no column named "SampleID" was found in the ASV count table. You don't *have* to follow this convention, but to use this package, it will make things much easier if you do. 
...
Like the previous example, this represents another mismatch between the ASV count and taxonomy tables. Again, if this were to happen with your data, make sure you're using the correct data and that it's not messed up somehow. Re-run the data processing if necessary.
...
Since R can't find a match between the metadata and ASV objects, you won't be able to do any actual analysis with your metadata. This would need to be fixed before moving on. 
